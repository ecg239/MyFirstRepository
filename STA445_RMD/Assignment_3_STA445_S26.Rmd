...
prob1c.fun(4,5)
```
Yes I did get the correct result for both of them.
...
I already did this in 2a and the code works.
...
```{r}
duniformfast <- function(x, a, b){
  if(a>=b){
    stop()
  }
  ifelse(x<=b & x>=a, 1/(b-a),0)
}
```
```{r}
duniformfast(4,2,8)
...
```
```{r eval=FALSE}
microbenchmark::microbenchmark( duniformfast( seq(-4,12,by=.0001), 4, 8), times=100)
```

The fast version was easier to write and the fast one ran faster. 
...
```{r}
duniformfast3 <- function(x, a = 0, b = 1){
  if(a>=b){
    stop()
  }
  ifelse(x<=b & x>=a, 1/(b-a),0)
