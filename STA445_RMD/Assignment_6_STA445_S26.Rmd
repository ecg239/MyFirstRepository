...
```{r}
COI <- COI %>%
  mutate(Year = factor(Year,levels = c("FirstYear","Sophomore","Junior","Senior"), ordered = TRUE))
```
...
```{r}
blank <- c("Alice","Ford F150")
A <- rbind(A,blank)
cbind(A,B[2])
```
```{r}
AB <- left_join(A,B, by = c("Name" = "First.Name"))
```
Using the dplyr function left_join was easier because I didn't have to code anything else. Cbind is more likely to have an error.
...
```{r}
MaxTemp <- read.csv("https://raw.githubusercontent.com/BuscagliaR/STA_444_v2/master/data-raw/FlagMaxTemp.csv")
MaxTemp
```

```{r}
MaxTemp_long <- MaxTemp %>%
  pivot_longer(
    cols = starts_with("X"),
    names_to = "Day",
    values_to = "MaxTemp"
  ...
  mutate(
    Day = as.numeric(gsub("X", "", Day)),   
    Date = make_date(Year, Month, Day)
  )
MaxTemp_2005 <- MaxTemp_long %>%
  filter(Year == 2005)
ggplot(MaxTemp_2005, aes(x = Date, y = MaxTemp)) +
  geom_line() +
  labs(
    title = "Daily Maximum Temperature in Flagstaff (2005)",
    x = "Date",
    y = "Max Temperature (°F)"
  ) +
  theme_bw()
...
ggplot(MaxTemp_monthly, aes(x = Date, y = AvgMaxTemp)) +
  geom_line() +
  labs(
    title = "Monthly Average Maximum Temperature (2013–2015)",
    x = "Date",
...
```{r}
va_flights <- flights %>%
  filter(month == 2, day == 14, carrier == "VX") %>%   # Virgin America = "VX"
  select(carrier, dest, dep_time, air_time)
va_flights_joined <- va_flights %>%
